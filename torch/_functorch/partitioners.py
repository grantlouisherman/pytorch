...
def is_getitem_of_multi_output(node: fx.Node) -> bool:
    if node.target != operator.getitem:
        return False
    parent = node.args[0]
    return "tensor_meta" not in parent.meta and node.op == "call_function"  # pyrefly: ignore [missing-attribute]
...
def solve_min_cut(
    ...
    def can_fuse_into_auto_functionalized(a: fx.Node, b: fx.Node) -> bool:
        ...
        (
            mutable_arg_names,
            _,
        ) = torch._higher_order_ops.auto_functionalize.get_mutable_args(mutable_op)  # pyrefly: ignore [bad-argument-type]
    ...
    def can_fuse_into_triton_kernel_wrapper_functional(a: fx.Node, b: fx.Node) -> bool:
        ...
        for name in mutable_arg_names:  # pyrefly: ignore [not-iterable]
            arg = b.kwargs["kwargs"][name]  # pyrefly: ignore [bad-index]
            if a is arg:
                return True
        return False
    ...
    def should_ban_recomputation(node: fx.Node) -> str | None:
        ...
        if node.dist_from_bw < 1000 and node.dist_from_bw > config.max_dist_from_bw:  # pyrefly: ignore [missing-attribute]
    ...
    def get_node_weight(
        ...
        mem_sz = int(mem_sz * (1.1 ** max(min(node.dist_from_bw, 100), 1)))  # pyrefly: ignore [missing-attribute]
...
def visualize_min_cut_graph(nx_graph: Any) -> tuple[str | None, str | None]:
